...
public class SocketIoController {
    ...
    public SocketIoController(SocketIOServer server) {
        ...
        server.addEventListener("createGameRoom", GameRoomRequestDto.CreateGameRoomRequestDto.class, (client, data, ackSender) -> GameRoomServiceImpl.createGameRoom(client, data));
    }

    public ConnectListener listenConnected() {
        return (client) -> {
            Map<String, List<String>> params = client.getHandshakeData().getUrlParams();
            log.info("datas: " + client.getHandshakeData().getHttpHeaders().toString());
